...
const AnomalyHistory = ({ setLoading, setError }) => { // 1. Destructure global loader props
  ...
  const loadHistory = async () => {
    // 2. Trigger Global Loader for initial vault sync
    setLoading(true);
    setError(null);
      ...
      <main className="pt-24 pb-20 px-4 md:px-8 space-y-8 max-w-7xl mx-auto overflow-x-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-3">
             ...
             <span className="text-[10px] font-black text-purple-400 uppercase tracking-[0.6em] italic">Forensic_Archives</span>
          </div>
          
          <div className="flex flex-wrap items-center gap-3">
            <button 
                onClick={() => navigate('/anomalies')} 
                className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest px-6 py-3 rounded-xl transition-all text-cyan-400 bg-cyan-500/5 border border-cyan-500/20 hover:bg-cyan-500/10 active:scale-95"
            >
            ...
            <div className="relative flex-1 md:flex-initial">
              ...
              <input 
                ...
                className="bg-gray-900/50 border border-gray-800 rounded-xl pl-10 pr-4 py-2.5 text-xs focus:border-purple-500 outline-none w-full md:w-64 uppercase tracking-widest transition-all" 
                value={searchTerm} 
        ...
        {/* FEED LIST - RESPONSIVE CARDS */}
        <div className="space-y-4 cyber-scroll max-h-[60vh] overflow-y-auto pr-2">
          {localSyncing ? (
            <div className="p-20 text-center text-gray-600 font-black uppercase tracking-[0.6em] animate-pulse">Syncing Vault...</div>
          ) : (
              ...
              >
                  <div className="flex items-center gap-4 md:gap-6 flex-1 min-w-0 w-full">
                     ...
                     <div className="min-w-0 flex-1">
                        <h4 className="text-lg md:text-xl font-black tracking-tight uppercase group-hover:text-purple-400 transition-colors truncate italic italic">
                            {log.process || 'SYSTEM_CORE'}
                        </h4>
                        <p className="text-[10px] text-gray-500 font-mono italic truncate mt-0.5 uppercase">
                            NODE :: {log.hostname} // {new Date(log.timestamp).toLocaleDateString()}
                        </p>
                        <div className="flex flex-wrap gap-3 mt-3">
                            <div className="flex items-center gap-1.5 opacity-40 shrink-0"><Clock size={10}/><span className="text-[9px] font-mono">{new Date(log.timestamp).toLocaleTimeString()}</span></div>
                            <div className="flex items-center gap-1.5 text-purple-500/60 font-mono text-[9px] min-w-0 uppercase tracking-tighter">
                              <Binary size={10} className="shrink-0"/><span className="truncate">{log.process_chain?.join(' > ') || 'Execution_Chain'}</span>
                            </div>
                        </div>
                     </div>
                  </div>
                ...
                <div className="p-5 md:p-6 border-b border-white/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/[0.02]">
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h2 className="text-lg md:text-xl font-black uppercase tracking-tight text-white truncate italic">{selectedLog.process}</h2>
                  ...
                  {showRawLog ? (
                    ...
                    <>
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                        <div className="md:col-span-4 bg-white/[0.03] border border-white/5 p-6 md:p-8 rounded-3xl flex flex-col justify-center min-h-[140px] shadow-inner">
                          ...
                          <span className="text-4xl md:text-5xl font-black text-white leading-none">{selectedLog.risk_score}%</span>
                        ...
                        <div className="md:col-span-8 bg-white/[0.03] border border-white/5 p-6 md:p-8 rounded-3xl min-h-[140px] overflow-hidden flex flex-col">
                            ...
                            {selectedLog.process_chain?.map((proc, idx) => (
                              <React.Fragment key={idx}>
                                <div className="flex flex-col items-center gap-1 shrink-0">
                                  <div className={`p-2.5 rounded-xl border backdrop-blur-sm ${idx === selectedLog.process_chain.length - 1 ? "border-red-500 bg-red-500/10 text-red-500 shadow-[0_0_10px_red]" : "border-white/10 bg-white/5 text-gray-500"}`}><Cpu size={14} /></div>
                                  <span className="text-[8px] font-mono uppercase text-gray-300 tracking-tighter">{proc}</span>
                                </div>
                                {idx < selectedLog.process_chain.length - 1 && (
                                  <div className="flex items-center self-start pt-4 shrink-0">
                                     <div className="w-5 h-[1px] bg-white/10 border-t border-dashed" />
                                  </div>
                                )}
                              </React.Fragment>
                            ))}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 overflow-hidden">
                        <div className="md:col-span-7 space-y-3 min-w-0">
                          ...
                          <div className="bg-black/20 backdrop-blur-md border border-white/10 rounded-[2rem] p-5 shadow-inner">
                            <div className="space-y-3 max-h-[220px] overflow-y-auto cyber-scroll pr-2">
                              {ghostLoading ? <div className="p-10 text-center text-[9px] font-black uppercase text-orange-500 animate-pulse">Running FIM Sweep...</div> : ghostFiles.length > 0 ? ghostFiles.map((file, idx) => (
                        ...
                        <div className="md:col-span-5 space-y-3 min-w-0">
                          ...
                          <div className="bg-white/[0.03] backdrop-blur-md border border-white/5 p-5 rounded-[2rem] h-full flex flex-col shadow-inner">
...
const StatTile = ({ label, value, icon, color }) => (
  <div className="p-6 md:p-8 bg-white/[0.02] border border-white/5 rounded-[2rem] shadow-xl backdrop-blur-md border-b-4 transition-all hover:bg-white/[0.03]" style={{ borderColor: color === 'purple' ? '#8b5cf6' : '#ef4444' }}>
    ...
    <p className={`text-3xl md:text-4xl font-black tracking-tighter ${color === 'purple' ? 'text-purple-400' : 'text-red-500'}`}>{value}</p>
  </div>
);
