...
import { 
  ...
  ShieldAlert, Fingerprint, Zap, FileCode, Clock, Link2, History, 
  ShieldOff, Snowflake, Database, Gauge, Loader2, Ghost, ArrowLeft, 
  Globe, MapPin, Orbit, Server, Target, Code, Binary, AlertCircle
} from 'lucide-react';
...
const AnomalyHistory = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-20 px-4 pt-4 relative text-white font-sans selection:bg-purple-500/30">
      <style>{`
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          <div className="flex items-center gap-2 mb-2">
             <Orbit size={14} className="text-purple-500 animate-pulse" />
             <span className="text-[10px] font-black text-purple-400 uppercase tracking-[0.6em]">Forensic_Archives</span>
          </div>
          <h1 className="text-5xl font-black tracking-tighter uppercase italic bg-clip-text text-transparent bg-gradient-to-b from-white to-white/40">EXPLOIT_HISTORY</h1>
        </motion.div>
        
        <div className="flex flex-wrap items-center gap-3">
          <button onClick={() => navigate('/anomalies')} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest px-6 py-3 rounded-xl transition-all text-cyan-400 bg-cyan-500/5 border border-cyan-500/20 hover:bg-cyan-500/10">
            <ArrowLeft size={14} /> Live Telemetry
          </button>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 w-4 h-4" />
            <input type="text" placeholder="Search history..." className="bg-gray-900/50 border border-gray-800 rounded-xl pl-10 pr-4 py-2.5 text-xs focus:border-purple-500 outline-none w-full md:w-64 uppercase tracking-widest" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4 cyber-scroll max-h-[60vh] overflow-y-auto pr-2">
        {!loading && filteredData.map((log) => (
          <motion.div 
            key={log.event_id}
            initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            onClick={() => { setSelectedLog(log); setShowRawLog(false); fetchGhostData(log); }}
            className="group bg-white/[0.02] border border-white/5 hover:border-purple-500/30 p-5 rounded-[2rem] flex flex-col md:flex-row items-center justify-between gap-6 backdrop-blur-md transition-all relative overflow-hidden cursor-pointer"
          >
              <div className="flex items-center gap-6 flex-1 min-w-0 w-full">
                 ...
                 <div className="min-w-0 flex-1">
                    <h4 className="text-xl font-black tracking-tight uppercase group-hover:text-purple-400 transition-colors truncate">{log.process || 'SYSTEM_CORE'}</h4>
                    <p className="text-[10px] text-gray-500 font-mono italic truncate mt-0.5 uppercase">NODE :: {log.hostname} // {new Date(log.timestamp).toLocaleDateString()}</p>
              ...
              <div className="flex items-center gap-8 shrink-0">
                 <div className="text-right hidden sm:block">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="w-24 h-1 bg-white/5 rounded-full overflow-hidden"><div className={`h-full ${log.risk_score > 80 ? 'bg-red-500 shadow-[0_0_10px_red]' : 'bg-purple-500'}`} style={{ width: `${log.risk_score}%` }} /></div>
              ...
              <div className="p-6 border-b border-white/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/[0.02]">
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h2 className="text-xl font-black uppercase tracking-tight text-white truncate">{selectedLog.process}</h2>
              ...
              <div className="p-6 md:p-8 space-y-8 overflow-y-auto cyber-scroll flex-1">
                {showRawLog ? (
                  ...
                  <>
                    ...
                    {/* MIDDLE ROW: GHOST_SCAN & CORRELATION */}
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 overflow-hidden">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="p-5 bg-red-500/5 border border-red-500/10 rounded-2xl flex items-center justify-between shadow-xl min-w-0">
                        <div className="flex items-center gap-2 min-w-0">
                           ...
                           <span className="text-[9px] font-black uppercase text-red-400 tracking-widest truncate">{selectedLog.summary || 'Historical event data recorded'}</span>
                        </div>
                      </div>
...
const StatTile = ({ label, value, icon, color }) => (
  <div className="p-8 bg-white/[0.02] border border-white/5 rounded-[2rem] shadow-xl backdrop-blur-md border-b-4 transition-all hover:bg-white/[0.03]" style={{ borderColor: color === 'purple' ? '#8b5cf6' : '#ef4444' }}>
    ...
    <p className={`text-4xl font-black tracking-tighter ${color === 'purple' ? 'text-purple-400' : 'text-red-500'}`}>{value}</p>
  </div>
);
