...
const safeGet = (url, fallback, signal) =>
  api.get(url, { signal }).then(r => r.data).catch(() => fallback);
...
const Dashboard = () => {
  ...
  // Sync Header Identity
  useEffect(() => {
    setHeaderData?.({
      name: "Dashboard",
      desc: "Infrastructure_Operational_Link"
    });
  }, [setHeaderData]);
  ...
  const loadData = useCallback(async (isInitial = false) => {
    ...
    try {
      const [cpuData, memData, topicsData, ratesData, timescaleData, dockerData, uptimeData] = await Promise.all([
        safeGet(`/system/cpu/timeseries`, [], controller.signal),
        safeGet(`/system/memory/timeseries`, [], controller.signal),
        safeGet(`/kafka/topics`, {}, controller.signal),
        safeGet(`/kafka/topic-rates`, {}, controller.signal),
        safeGet(`/timescale/health`, { total_table_size: "0 MB", total_rows_logs: 0 }, controller.signal),
        safeGet(`/docker/health`, { docker_daemon: "down", containers_running: 0 }, controller.signal),
        safeGet(`/system/uptime`, { formatted: "0m" }, controller.signal)
      ]);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-cyan-400 animate-pulse">
      ...
      <AnimatePresence>
        {isError && (
          <motion.div 
            ...
            className="bg-red-500/10 border border-red-500/20 text-red-400 p-3 rounded-xl flex items-center gap-3 font-jetbrains text-xs overflow-hidden"
          >
            <AlertCircle size={16} />
