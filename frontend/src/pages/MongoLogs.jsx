...
import { Activity, Terminal, ListFilter, Copy } from "lucide-react";
...
const MongoLogs = () => {
  ...
  useEffect(() => {
    if (setHeaderData) {
      setHeaderData({
        name: "Raw Events",
        desc: "MongoDB_Unstructured_Log_Mesh // NoSQL Telemetry"
      });
    }
  }, [setHeaderData]);
  ...
  return (
    // 3. Removed min-h-screen/pt-24 (handled by layout)
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white/[0.02] p-4 rounded-2xl border border-white/5">
        ...
        <div className="flex items-center gap-2 font-roboto-condensed">
          <button
            ...
            className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-[10px] font-black uppercase disabled:opacity-20 hover:bg-white/10 transition-all text-white"
          >
            Prev_Sector
          </button>
          <button
            onClick={() => setOffset(offset + limit)}
            className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-[10px] font-black uppercase hover:bg-white/10 transition-all text-white"
          >
            Next_Sector
          </button>
        </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-white/5 font-jetbrains text-[11px]">
              {loading ? (
                ...
                <tr><td colSpan={3} className="p-10 text-center text-gray-700 uppercase tracking-widest">Buffer_Empty</td></tr>
              ) : logs.map((row, idx) => (
