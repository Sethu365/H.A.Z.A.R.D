      ...
      <main className="pt-24 pb-20 px-4 md:px-8 max-w-7xl mx-auto space-y-8 overflow-x-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div className="flex items-center gap-3">
             ...
             <span className="text-[10px] font-extrabold text-cyan-400 uppercase tracking-[0.6em] italic">Active_Nodes</span>
          </div>

          <div className="relative w-full sm:w-auto" ref={menuRef}>
            <button
              onClick={() => setShowAddMenu(!showAddMenu)}
              className="w-full sm:w-auto flex items-center justify-center gap-3 bg-white text-black px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all active:scale-95 shadow-xl hover:bg-cyan-400"
            >
            ...
            <AnimatePresence>
              {showAddMenu && (
                <motion.div
                  ...
                  className="absolute right-0 mt-3 w-56 bg-[#0a0c14] border border-white/10 rounded-[1.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] z-50 p-2 overflow-hidden backdrop-blur-xl"
                >
                  <div className="px-4 py-3 text-[8px] font-extrabold text-gray-500 uppercase tracking-[0.2em] border-b border-white/5 mb-1 italic">Protocol_Selection</div>
                  <button onClick={() => handleDownload("linux")} className="w-full flex items-center justify-between px-4 py-3 hover:bg-white/5 rounded-xl text-[11px] font-extrabold text-gray-200 transition-all group">
                    <div className="flex items-center gap-3">
                      <Terminal size={16} className="text-cyan-400" /> Linux_gRPC
                    </div>
                    <Download size={14} className="opacity-20 group-hover:opacity-100 transition-opacity" />
                  </button>
                  <button onClick={() => handleDownload("windows")} className="w-full flex items-center justify-between px-4 py-3 hover:bg-white/5 rounded-xl text-[11px] font-extrabold text-gray-200 transition-all group">
                    <div className="flex items-center gap-3">
                      <Monitor size={16} className="text-blue-400" /> Win_Sensor
                    </div>
                    <Download size={14} className="opacity-20 group-hover:opacity-100 transition-opacity" />
                  </button>
                </motion.div>
            ...
            >
              ...
              <div className="mb-8 flex justify-between items-start">
                <div className="min-w-0">
                  <h2 className={`text-2xl font-black italic tracking-tighter uppercase truncate ${client.isOnline ? 'text-white' : 'text-gray-600'}`}>
                    {client.hostname}
                  </h2>
                  <p className="text-[8px] font-extrabold text-gray-500 uppercase tracking-widest mt-1">Status_Registry</p>
                </div>

                <div className={`flex items-center gap-2 px-3 py-1 rounded-full border-2 text-[8px] font-extrabold uppercase tracking-widest italic ${
                  client.isOnline 
              ...
              <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between">
                <div className="flex flex-col">
                    <span className="text-[7px] font-extrabold text-gray-600 uppercase tracking-widest mb-1">Last_Pulse</span>
                    <p className="text-[10px] font-extrabold text-gray-400 italic">
